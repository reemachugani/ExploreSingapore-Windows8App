...
(function () {
    ...
    ui.Pages.define(searchPageURI, {
        ...
        generateFilters: function () {
            ...
            this.filters.push({ results: null, text: "Outdoors", predicate: function (item) { return item.group.key === "group1"; } });
            this.filters.push({ results: null, text: "Food", predicate: function (item) { return item.group.key === "group2"; } });
            this.filters.push({ results: null, text: "Drinks", predicate: function (item) { return item.group.key === "group3"; } });
            this.filters.push({ results: null, text: "Coffee", predicate: function (item) { return item.group.key === "group4"; } });
            this.filters.push({ results: null, text: "Shopping", predicate: function (item) { return item.group.key === "group5"; } });
            this.filters.push({ results: null, text: "Arts", predicate: function (item) { return item.group.key === "group6"; } });
        },

        itemInvoked: function (args) {
            args.detail.itemPromise.done(function itemInvoked(item) {
            //    // TODO: Navigate to the item that was invoked.
                //    var idx = Data.items.indexOf(item.data);
                var idx = Data.items.getAt(item.data);
                //var item = this.items.getAt(args.detail.itemIndex);
                nav.navigate("/pages/spotsItemDetail/spotsItemDetail.html", {group: item.data.group, selectedIndex: idx
                });   
            });
        },
